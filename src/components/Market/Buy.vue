<template>
  <div id="subMenu" ref="subMenu">
    ...
    <app-control v-bind:head="language === 'RU' ? 'Покупка: ' : 'Purchase: ' + order.Item.name"
    ...
    <div class="wrapper">
      ...
      <div>
        <span style="float: left"> {{ language === 'RU' ? 'Купить' : 'Buy count' }} </span>
        <input id="buyCount" v-model="quantity" style="float: right" name="count" type="number" min="1"
               :max="order.Count">
      </div>

      <div>
        <span style="float: left"> {{ language === 'RU' ? 'Всего кредитов' : 'Total credits' }} </span>
        <output style="float: right" name="result">{{ order.Price * quantity }}</output>
      </div>

      <input type="button" @click="back" class="lobbyButton inventoryTip"
             :value="language === 'RU' ? 'Отменить' : 'Cancel'">
      <input type="button" @click="buy" class="lobbyButton inventoryTip"
             :value="language === 'RU' ? 'Купить' : 'Buy'">
    </div>
  </div>
</template>
